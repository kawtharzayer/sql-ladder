...
SELECT  "lastname" || ', ' ||"firstname" AS fullname,ROUND(salary / 1.1, 2) AS salary_eu
FROM "employees"
...
SELECT "ID", "lastname" || ', ' ||"firstname" AS fullname,"job" ,
...
PRINTF('$%,d', salary) AS salary,"startdate", 
PRINTF('$%,d', CASE
when "job" like "sales" and "salary">100000 then salary*1.1
when ("job" like "sales" or "job" like "Administrators" ) and "salary"<100000 then salary*1.05
else salary 
end )as target_comp 
from "employees"
